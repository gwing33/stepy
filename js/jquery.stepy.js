...
;(function($) {
  ...
  $.fn.stepy = function(settings) {
    ...
    var opt           = $.extend({}, $.fn.stepy.defaults, settings),
      ...
      size            = $steps.size(),
      $titlesWrapper  = $('<ul/>', { 'class': 'stepy-titles' }),
      description     = '',
      title           = '',
      $legend         = null,
      isForm          = $this.is('form'),
    ...
    if (opt.titleTarget) {
      $titlesWrapper = opt.titleTarget;
      opt.titlesWrapperId = opt.titleTarget.attr('id');
    } else {
      opt.titlesWrapperId = id + '-titles';
      $titlesWrapper.attr('id', opt.titlesWrapperId).insertBefore($this);
    }
  ...
  function selectStep(context, index, isBlocked, opt) {
    var id    = context.attr('id'),
      ...
      size  = $steps.size(),
      $titles = $('ul#' + opt.titlesWrapperId).children();
  ...
  function validate(context, index, opt) {
    ...
    var id    = context.attr('id'),
      ...
      $title  = $('ul#' + opt.titlesWrapperId).children().eq(index);
    ...
    selectStep(context, index - 1, false, opt);
